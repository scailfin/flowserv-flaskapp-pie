...
"""Blueprint for workflow runs and run results."""

import os

from flask import Blueprint, jsonify, make_response, request, send_file

from flaskflow.api.auth import ACCESS_TOKEN
from flaskflow.api.util import jsonbody
from flaskflow.service import service

import flowserv.config.api as config
import flowserv.util as util
import flaskflow.error as err
...
@bp.route('/workflows/<string:workflow_id>/runs', methods=['GET'])
def list_runs(workflow_id):
    ...
    """
    # Get the access token. We do not require an access token to be present in
    # the header for configurations where the authentication policy is open.
...
@bp.route('/workflows/<string:workflow_id>/runs', methods=['POST'])
def start_run(workflow_id):
    ...
    Raises
    ------
    flaskflow.error.InvalidRequest
    flowserv.error.UnauthenticatedAccessError
    flowserv.error.UnauthorizedAccessError
    flowserv.error.UnknownWorkflowGroupError
    """
...
@bp.route('/workflows/<string:workflow_id>/runs/poll', methods=['GET'])
def poll_runs(workflow_id):
...
@bp.route('/runs/<string:run_id>/files/downloads/archive')
...
@bp.route('/runs/<string:run_id>/files/downloads/resources/<string:file_id>')
